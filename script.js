...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initMacroControls();
    renderWeight();
    renderCardioChart(); // Ajout du tracé de la courbe cardio
    renderRecords();
    renderCardio();      // Affichage du tableau (liste) cardio
    renderCalendarInit();
    updateSmartCoach();
    updateGamification();
});
...
function renderCardioChart() {
    ...
    if (history.length === 0) {
        container.innerHTML = '<p class="bento-item-desc" style="text-align:center; padding: 1rem;">Aucune donnée cardio pour tracer la courbe.</p>';
        return;
    }
    
    // Trier par date croissante pour l'axe chronologique de la courbe
    ...
    const distances = sorted.map(x => x.dist);
    const minD = Math.max(0, Math.min(...distances) - 2);
    const maxD = Math.max(...distances) + 2;
    ...
    for(let i=0; i<=4; i++) {
        let y = paddingY + (i * ((h - 2*paddingY)/4));
        let val = (maxD - (i * (range/4))).toFixed(1);
        svg += `<line x1="${paddingX}" y1="${y}" x2="${w}" y2="${y}" stroke="var(--border)" stroke-dasharray="4" stroke-width="1" />`;
        svg += `<text x="${paddingX - 10}" y="${y+4}" fill="var(--text-muted)" font-size="10" text-anchor="end">${val} km</text>`;
    }
    
    const points = sorted.map((entry, i) => {
        const stepX = (w - paddingX) / Math.max(1, sorted.length - 1);
        const x = paddingX + (i * stepX);
        const y = paddingY + (h - 2*paddingY) - ((entry.dist - minD) / range * (h - 2*paddingY));
        return { x, y };
    });
    
    // Tracé de la courbe lié en bleu
    if (points.length > 1) {
        let pathD = `M ${points[0].x} ${points[0].y}`;
        for (let i = 1; i < points.length; i++) {
            pathD += ` L ${points[i].x} ${points[i].y}`;
    ...
    points.forEach(p => {
        svg += `<circle cx="${p.x}" cy="${p.y}" r="5" fill="var(--navy)" stroke="#3b82f6" stroke-width="2" />`;
    });
